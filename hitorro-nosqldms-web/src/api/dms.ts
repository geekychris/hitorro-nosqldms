...
export interface FieldDef {
  name: string;
  kind: string;             // 'string' | 'text' | 'long' | 'double' | 'boolean' | 'date' | 'url' | 'enum' | 'array<string>'
  label?: string;
  help?: string;
  required?: boolean;
  choices?: string[];       // for enum
  rows?: number;            // for text
}
...
export const dms = {
  ...
  types: (): Promise<TypeDef[]> =>
    fetch('/api/types').then(json),

  type: (name: string): Promise<TypeDef> =>
    fetch(`/api/types/${name}`).then(json),
};
